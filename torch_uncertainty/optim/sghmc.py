...
class SGHMC(Optimizer):
    def __init__(
        self, params: ParamsT, lr: float = 1e-2, friction: float = 0.05, weight_decay: float = 0
    ) -> None:
        ...
        defaults = {"lr": lr, "friction": friction, "weight_decay": weight_decay}
        super().__init__(params, defaults)

    def step(self, closure: Callable[[], float] | None = None, burn_in: bool = False) -> float:
        loss = None
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                noise_std = torch.clamp(noise_var, min=1e-16).sqrt()
                # sample random noise
