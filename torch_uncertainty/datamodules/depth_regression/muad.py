...
class MUADDataModule(AbstractDataModule):
        ...
        self.train_transform = v2.Compose(
            [
                ...
                v2.RandomCrop(
                    size=self.crop_size,
                    pad_if_needed=True,
                    fill={tv_tensors.Image: 0, tv_tensors.Mask: -float("inf")},
                ),
    ...
    def prepare_data(self) -> None:  # coverage: ignore
        self.dataset(
            root=self.root, split="train", target_type="depth", download=True
        )
        self.dataset(
            root=self.root, split="val", target_type="depth", download=True
        )
