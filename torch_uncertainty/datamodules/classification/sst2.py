...
class Sst2DataModule(TUDataModule):
    ...
    def __init__(self, model_name="bert-base-uncased", max_len=128, batch_size=32,
                 eval_batch_size=None, num_tta=1, num_workers=4,
                 pin_memory=True, persistent_workers=True,
                 local_files_only=False, eval_ood: bool = True):
        super().__init__(
            root=".",
    ...
    def _tok_single(self, raw, field, name):
        def tok(b):
            return self.tokenizer(b[field], max_length=self.max_len, truncation=True, padding="max_length")
    ...
    def _tok_pair(self, raw, field1, field2, name):
        def tok(b):
            return self.tokenizer(b[field1], b[field2], max_length=self.max_len,
    ...
    def _tok_wmt16_en(self, raw, name):
        def tok(b):
            ...
            return self.tokenizer(en, max_length=self.max_len, truncation=True, padding="max_length")
    ...
    def setup(self, stage=None):
        ...
        tokds.set_format(type="torch",
                         columns=[c for c in ["input_ids", "attention_mask", "label"]
        ...
        if stage in (None, "test"):
            ...
            if self.eval_ood:
                ...
                mnli_raw   = load_dataset("glue", "mnli",           split="validation_mismatched", download_config=dl)
    ...
    def get_indices(self):
        ...
        out = {"test": [idx]}; idx += 1
        if self.eval_ood:
            out["test_ood"]  = [idx]; idx += 1
            out["val_ood"]   = []  # kept empty
            out["near_oods"] = list(range(idx, idx + len(self.near_oods))); idx += len(self.near_oods)
            out["far_oods"]  = list(range(idx, idx + len(self.far_oods)));  idx += len(self.far_oods)
        else:
