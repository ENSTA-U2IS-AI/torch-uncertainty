...
class Zero(nn.Module):
    def __init__(self, model: nn.Module, num_tta: int, filter_views: float = 0.1) -> None:
        ...
        """
        super().__init__()
        _zero_checks(num_tta, filter_views)
    ...
    def eval_forward(self, x: Tensor) -> Tensor:
        ...
        entropies = entr(all_predictions).sum(1)
        ...
        for img_id, img_votes in enumerate(votes):
            ...
            i = 0
            # If the maximum corresponds to two predictions, look at an additional
            while i < self.num_tta and torch.sum(1 * (predictions[img_id, :] == maximum)) > 1:
...
def _zero_checks(num_tta: int, filter_views: float) -> None:
