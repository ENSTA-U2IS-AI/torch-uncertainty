...
from torch_uncertainty.post_processing import ConformalClsTHR, ConformalClsAPS, ConformalClsRAPS
...
#Please note that this not necessary usefull for conformal, but since it is easy with TU
...
conformal_model = ConformalClsTHR(model, device="cuda", alpha=0.1)
...
def visualize_prediction_sets(inputs, labels, prediction_sets, confidence_scores, classes, num_examples=5):
...
visualize_prediction_sets(inputs, labels, prediction_sets[:5].cpu().numpy(), confidence_scores[:5].cpu().numpy(), classes)

# %%
# 7. Calibrate with ConformalClsAPS
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
print("\n[Phase 3]: ConformalClsAPS calibration")
conformal_model = ConformalClsAPS(model, device="cuda", alpha=0.1)
...
conformal_model = ConformalClsRAPS(model, device="cuda", alpha=0.1)
