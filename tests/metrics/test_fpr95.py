...
class TestFPR95:
    ...
    def test_compute_zero(
        ...
        self.metric = FPR95(pos_label=1)
        self.metric.update(confs_zero, target_zero)
    ...
    def test_compute_half(
        ...
        self.metric = FPR95(pos_label=1)
        self.metric.update(confs_half, target_half)
        res = self.metric.compute()
        assert res == 0.5
    ...
    def test_compute_one(
        ...
        self.metric = FPR95(pos_label=1)
        self.metric.update(confs_one, target_one)
        res = self.metric.compute()
        assert res == 1
